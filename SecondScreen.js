...
export class SecondScreen extends Component<Props> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      hideControls: false,
      video: 'Change video',
      path: require('./videos/1_video.mp4'),
    };
  ...
  // on press change video event
  onPressBtnChangeVideo() {
    const video1 = require('./videos/1_video.mp4');
    const video2 = require('./videos/2_video.mp4');

    if (this.state.video === 'Change video') {
      this.setState({
        video: 'Video 2',
        path: video2,
      });
    } else if (this.state.video === 'Video 2') {
      this.setState({
        video: 'Video 1',
        path: video1,
      });
    } else if (this.state.video === 'Video 1') {
      this.setState({
        video: 'Video 2',
        path: video2,
      });
          ...
          <View style={styles.controls}>
            <View style={styles.generalControls}>
              <View style={styles.rateControl}>
                <Text
                  style={{width: 110}}
                  onPress={() => this.onPressBtnChangeVideo()}>
                  {this.state.video}
                </Text>
              </View>
              ...
              <View style={styles.resizeModeControl}>
                ...
                <Text style={{width: 110}} onPress={() => navigate('First')}>
